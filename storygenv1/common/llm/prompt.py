...
class PromptBuilder:
    def display(self):
        ...
        print("Response Prefix:", self.response_prefix) 
    ...
    def render_for_llm_format(self, prompt_format):
        if prompt_format not in ['openai-chat', 'llama2-chat', 'together-chat', 'none']:
            raise NotImplementedError(f"Prompt format {prompt_format} not implemented.")

        prompt = self.instruction.format().lstrip()

        if prompt_format in ['openai-chat', 'together-chat']:
...
def load_prompts(path, file_name):
    with open(path / file_name) as f:
        prompts = json.load(f)
    _create_prompt_templates(prompts)
    return prompts

def load_premise_prompts(path, file_name, user_prompt):
    with open(path / file_name) as f:
        prompts = json.load(f)

    prompts['title']['instruction'] += user_prompt
    prompts['premise']['instruction'] += user_prompt
